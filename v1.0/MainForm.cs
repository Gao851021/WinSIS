...
namespace WinSIS
{
    public partial class MainForm : Form
    {
        ...
        private string[] detail = new string[8];
        private string[] componentName = new string[8];
        ...
        private void MainForm_Shown(object sender, EventArgs e)
        {
            ...
            catch (Exception) { MessageBox.Show("ERROR: MISSING SETTINGS FILE"); this.Close(); };

            txtOpSystem.Text = LangValue[112];
            txtMotherboard.Text = LangValue[112];
            txtProcessor.Text = LangValue[112];
            txtPeripherals.Text = LangValue[112];
            ...
            linkPeripherals.Text = LangValue[120]; // Peripherals
            ...
            /* Peripherals summary information */
            componentName[7] = LangValue[120]; // Peripherals

            i = 0;
            byte j = 0, k = 0;
            foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Description FROM Win32_Keyboard").Get()) { i++; };
            foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Description FROM Win32_PointingDevice").Get()) { j++; };
            foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Caption FROM Win32_Printer").Get()) { k++; };

            switch (i)
            {
                case 0: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[121], LangValue[53]); break;
                default: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[121], LangValue[52]); break;
            }
            switch (j)
            {
                case 0: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[122], LangValue[53]); break;
                default: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[122], LangValue[52]); break;
            }
            switch (k)
            {
                case 0: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[154], LangValue[53]); break;
                default: info[7] += String.Format("[{0}]:  {1}\r\n", LangValue[154], LangValue[52]); break;
            }
            txtPeripherals.Text = info[7];
        ...
        private void GetDetails()
        {
            ...
            /* Details about the peripherals */

            detail[7] += String.Format("{0}      {1}{2}", shortbreak, LangValue[120], shortbreak); // Peripherals
            try
            {
                foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Caption, PNPClass, Manufacturer, Status FROM Win32_PnPEntity").Get())
                {
                    switch (Convert.ToString(obj["PNPClass"]))
                    {
                        case "Keyboard":
                            detail[7] += String.Format("{0}\r\n", obj["Caption"]);
            ...
            {
                foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Caption, DeviceID, Status FROM Win32_USBHub").Get()) // USB Devices
                {
                    detail[7] += String.Format("{0}\r\n", obj["Caption"]);
                    detail[7] += String.Format("    [{0}]:  {1}\r\n", LangValue[76], obj["DeviceID"]);
                    detail[7] += String.Format("    [{0}]:  {1}\r\n", LangValue[58], obj["Status"]);
                }
            }
            detail[7] += String.Format("{0}      {1}{2}", shortbreak, LangValue[57], shortbreak);
            foreach (ManagementObject obj in new ManagementObjectSearcher("SELECT Caption, Default, Network, ExtendedPrinterStatus FROM Win32_Printer").Get())
            {
                detail[7] += String.Format("{0}\r\n", obj["Caption"]);
                detail[7] += String.Format("    [{0}]:  ", LangValue[143]); // Default
                switch (Convert.ToBoolean(obj["Default"]))
                {
                    case true: detail[7] += String.Format("{0}\r\n", LangValue[144]); break; // Yes
                    case false: detail[7] += String.Format("{0}\r\n", LangValue[145]); break; // No
                }
                detail[7] += String.Format("    [{0}]:  ", LangValue[82]); // Network
                switch (Convert.ToBoolean(obj["Network"]))
                {
                    case true: detail[7] += String.Format("{0}\r\n", LangValue[146]); break; // Network
                    case false: detail[7] += String.Format("{0}\r\n", LangValue[147]); break; // Local
                }
                detail[7] += String.Format("    [{0}]:  ", LangValue[58]); // Status
                switch (Convert.ToByte(obj["ExtendedPrinterStatus"]))
                {
                    case 1: detail[7] += String.Format("{0}", LangValue[59]); break; // Other
                    case 2: detail[7] += String.Format("{0}", LangValue[37]); break; // Unknown
                    case 3: detail[7] += String.Format("{0}", LangValue[128]); break; // Idle
                    case 4: detail[7] += String.Format("{0}", LangValue[129]); break; // Printing
                    case 5: detail[7] += String.Format("{0}", LangValue[130]); break; // Warming up
                    case 6: detail[7] += String.Format("{0}", LangValue[131]); ; break; // Stopped printing
                    case 7: detail[7] += String.Format("{0}", LangValue[132]); ; break; // Offline
                    case 8: detail[7] += String.Format("{0}", LangValue[133]); break; // Paused
                    case 9: detail[7] += String.Format("{0}", LangValue[134]); break; // Error
                    case 10: detail[7] += String.Format("{0}", LangValue[135]); break; // Busy
                    case 11: detail[7] += String.Format("{0}", LangValue[53]); break; // Unavailable
                    case 12: detail[7] += String.Format("{0}", LangValue[136]); break; // Waiting
                    case 13: detail[7] += String.Format("{0}", LangValue[137]); break; // Processing
                    case 14: detail[7] += String.Format("{0}", LangValue[138]); break; // Initialization
                    case 15: detail[7] += String.Format("{0}", LangValue[139]); break; // Power save
                    case 16: detail[7] += String.Format("{0}", LangValue[140]); break; // Pending deletion
                    case 17: detail[7] += String.Format("{0}", LangValue[141]); break; // I/O active
                    case 18: detail[7] += String.Format("{0}", LangValue[142]); break; // Manual feed
                }
                detail[7] += "\r\n";
            }

            this.Invoke(new MethodInvoker(() => linkPeripherals.Enabled = true));
        ...
        private void SaveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (save.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    sw.WriteLine("\t--------------------------------");
                    sw.WriteLine("\tWinSIS | Version 1.0");
